...
import { http, runtimeState } from "@blitzpress/plugin-sdk";
...
async function checkAuthStatus(): Promise<AuthStatus> {
  const res = await http().asJson().get("/api/core/auth/status");
  if (!res.ok) return { enabled: false };
  return res.json();
}

async function fetchCurrentUser(): Promise<AuthUser | null> {
  const res = await http().asJson().get("/api/core/auth/me");
...
export const AdminRuntimeProvider: Component<{ children: JSX.Element }> = (props) => {
  ...
  const logout = async () => {
    const res = await http().asJson().post("/api/core/auth/logout");
