...
describe("plugin loader", () => {
  test("reports manifest validation failures for frontend plugins with missing entry", async () => {
  ...
  test("loads a valid frontend plugin without injecting a stylesheet when style is missing", async () => {
    const fetchMock = async () =>
      new Response(
        JSON.stringify({
          plugins: [
            {
              ...
              version: "1.0.0",
              has_frontend: true,
              frontend_entry: "/plugins/users-plugin/assets/index.js",
            },
          ],
        }),
        {
    ...
    expect(summary.loaded).toEqual(["users-plugin"]);
    expect(summary.failed).toEqual([]);
    expect(importer).toHaveBeenCalledWith("/plugins/users-plugin/assets/index.js");
    expect(appendChild).not.toHaveBeenCalled();
  });
});
